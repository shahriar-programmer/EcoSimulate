...
export default function Header({ page, setPage }: Props) {
  ...
  return (
    <div className='flex items-center justify-between w-full h-20 bg-gradient-to-l from-[#000046] to-[#1CB5E0] px-44'>
      ...
      <div className='flex text-lg font-semibold text-white gap-x-8'>
        {
          pagesArray.map((pA, index) => (
            <p key={index} onClick={() => setPage(pA.path)} className={`px-6 py-1 transition ${page == pA.path ? "bg-blue-400/30" : "bg-transparent"} border rounded hover:bg-blue-400/30 cursor-pointer`}>{pA.title}</p>
          ))
        }
      </div>
    </div>
